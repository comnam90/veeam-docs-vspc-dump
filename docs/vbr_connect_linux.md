---
title: "Connecting Linux Veeam Backup & Replication Servers"
product: "vspc"
doc_type: "provider_admin"
source_url: "https://helpcenter.veeam.com/docs/vac/provider_admin/vbr_connect_linux.html"
last_updated: "4/29/2026"
product_version: "9.2.0.33215"
---

# Connecting Linux Veeam Backup & Replication Servers


To deploy a management agent on the Linux Veeam Backup & Replication server (Veeam Software Appliance), you must enable remote data collection in the Veeam Host Management Console. For details, see [Configuring Backup Infrastructure Settings](https://helpcenter.veeam.com/docs/vbr/userguide/hmc_configure_infrastructure.html?ver=13) in the Veeam Backup & Replication User Guide.

|  |
| --- |
| Note: |
| Some services may restart during management agent deployment. As a result, running jobs may finish with an error. To avoid this, make sure to stop all running jobs before deploying the management agent. |

To configure a connection to the Linux Veeam Backup & Replication server:
...
5. At the top of the server list, click Add and select Linux Backup Server.
...
If the Veeam Backup & Replication server is a primary node of a High Availability cluster, Veeam Service Provider Console will automatically register the server as a cluster node. After the primary node is connected, you must repeat steps 6–10 to connect the secondary node of this cluster.

Alternatively, you can connect the secondary node on the Discovery > Backup Servers tab. For details, see [Managing Backup High Availability Clusters](vbr_ha_clusters.md#connect).

1. Repeat steps 6–10 for all Veeam Backup & Replication servers that you want to add.
