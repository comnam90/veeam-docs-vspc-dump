...
source_url: "https://helpcenter.veeam.com/docs/vac/provider_admin/sso_idp.html"
...
* [For Okta, Keycloak and Custom identity providers] In the Entity ID field, specify the unique identifier that an IdP will use to identify SP.
...
* [For Okta, Keycloak and Custom identity providers] Click SP entity ID link to generate entity ID URL based on the portal web address and the Client ID value.

If you apply changes to Client ID value after link generation, click New link.

* [For Okta, Keycloak and Custom identity providers] Click Create Assertion consumer link to generate assertion consumer service URL based on the portal web address and the Client ID value.
...
1. Modify identity provider settings as described in [Adding Identity Providers](#add_idp).
2. Click Finish.
