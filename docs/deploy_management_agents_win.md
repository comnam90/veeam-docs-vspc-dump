...
If you have downloaded the management agent setup file from the Veeam Service Provider Console and the management agent is assigned to a company, it will automatically connect to Veeam Service Provider Console with the assigned company configuration. Otherwise, you must configure the agent manually.
...
* For agents hosted in client infrastructure, including master agents that will be used for discovery:
...
* For agents hosted in your infrastructure, including master agents that will be used for discovery:

1. In the Backup portal field, type FQDN or IP address of the Veeam Service Provider Console portal.
2. In the Port field, specify the port that is used to transfer data to Veeam Service Provider Console.
3. [Optional] In the Tag field, specify the tag that must be assigned to the management agent.

[![Configure Management Agent](images/configure_master_agent_hosted.webp)](images/configure_master_agent_hosted.webp "Configure Management Agent")
