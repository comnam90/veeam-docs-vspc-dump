...
source_url: "https://helpcenter.veeam.com/docs/vac/provider_admin/export_vbr_details.html"
...
To manage Veeam Backup & Replication servers, a user must have one of the following roles assigned: Portal Administrator, Site Administrator, Portal Operator, Read-only User.
...
You can click a link in the Update Status column to view patching or upgrading progress, download task session logs or cancel patching or upgrading. For details, see [Applying Patches to Veeam Backup & Replication Servers](vbr_patch.md).

[For all roles except for Read-only User] If patching or upgrading was canceled and the Update Status is Error, click Clear Logs to update the status.
