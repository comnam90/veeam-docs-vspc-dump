...
Before you connect Veeam Backup & Replication, obtain the following data from the service provider:

* FQDN or IP address of a cloud gateway.
* Port on the cloud gateway used to transfer backup data to and from cloud repositories. The management agent will use this port for communication with Veeam Service Provider Console.

Make sure this port is open on the machine hosting Veeam Backup & Replication.
...
You must specify credentials of the Company Tenant communicated to you by the service provider.
...
The management agent on the Veeam Backup Enterprise Manager server must be deployed manually. For details, see [Deploying Management Agents Manually](deploy_management_agents.md).
