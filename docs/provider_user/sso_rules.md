...
source_url: "https://helpcenter.veeam.com/docs/vac/provider_user/sso_rules.html"
...
To perform the following tasks, a user must have  the following role assigned: Company Owner, Company Administrator.
...
For details on user roles, see [User Roles and Permissions](user_roles_permissions.md).

1. At the next step, select locations that must be assigned to a user:
...
| --- |
| Important! |
| For IdP to successfully identify authorizing users, you must provide an email address of each user in the IdP service and in Veeam Service Provider Console. For details on how to add email address to a user profile in Veeam Service Provider Console, see [Modifying User Profile](modify_user_profile.md#mfa_config). |

|  |
| --- |
| Note: |
| For Company Owner, new identity will not be created if matching identity is not found. To avoid authorization issues, make sure that email address is specified in the company profiles as described in the [Filling Company Profile](fill_tenant_profile.md) section. |
